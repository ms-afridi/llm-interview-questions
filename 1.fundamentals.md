...
### Q16: What is the difference between model parameters and hyperparameters?

**Answer:**

**Parameters** are learned during training, while **hyperparameters** are set before training and control how learning happens.

**Parameters (Learned):**
- Values the model discovers through training
- Adjusted automatically via backpropagation
...
Storage: 4 bytes per parameter (FP32)
```

**Hyperparameters (Set by humans):**
- Configuration choices made before training
...
- Number of layers: 12, 24, 96
- Hidden size: 768, 1024, 12288
- Number of attention heads: 8, 16, 96
- Context window: 2048, 4096, 8192

Training Hyperparameters:
- Learning rate: 0.0001, 0.001
- Batch size: 32, 64, 256
- Number of epochs: 3, 10, 100
...
model = GPT(
    num_layers=12,        # Hyperparameter
    hidden_size=768,      # Hyperparameter
    num_heads=12,         # Hyperparameter
    vocab_size=50000      # Hyperparameter
)

# This creates millions of PARAMETERS
# Total params: ~117M (calculated from hyperparameters)

# TRAINING HYPERPARAMETERS
training_config = {
    'learning_rate': 0.0001,    # Hyperparameter
    'batch_size': 64,           # Hyperparameter
    'epochs': 3                 # Hyperparameter
}

# During training, the 117M PARAMETERS get updated
# The hyperparameters stay fixed
```

**Real Interview Scenario:**

```
Interviewer: "Your model isn't performing well. What do you tune?"

Bad Answer: "I'll change the parameters"
(Parameters are learned, you don't manually change them!)

Good Answer: "I'll tune hyperparameters:
1. Learning rate - might be too high/low
2. Batch size - affects convergence
3. Model architecture - might need more layers
4. Training data - might need more/better quality
Then retrain to learn new parameters."
```
...
----------      ------------------      -------------------
Set by          Training algorithm      Human/AutoML
When            During training         Before training
Count           Billions               Dozens
Examples        Weights, biases         Learning rate, layers
Saved           In model file          In config file
Tuning          Automatic (gradient)    Manual (trial/error)
```

**Interview Tip:** When someone asks "how many parameters does GPT-3 have?", they mean learned parameters (175B). When they ask "what hyperparameters did you use?", they want training configuration details.

---

### Q17: What is temperature in LLM generation? How does it affect outputs?

**Answer:**

Temperature is a hyperparameter that controls the randomness/creativity of the model's text generation. It adjusts the probability distribution over next token predictions.

**How It Works:**

After the model computes logits (raw scores) for each possible next token, temperature scales these before converting to probabilities:
...
Output (every time): "Paris"
```

**Temperature = 0.7 (Balanced - Default for ChatGPT):**
- Good balance of coherence and creativity
- Slightly random but sensible

```
Prompt: "Write a story opening"
Output 1: "The old lighthouse stood guard over the rocky shore..."
Output 2: "On a misty morning, the village awoke to strange sounds..."
Output 3: "Sarah had always wondered what lay beyond the forest..."
```

**Temperature = 1.0 (Standard):**
- Uses raw probability distribution
- More random and diverse

**Temperature = 2.0 (Very Creative/Random):**
- High randomness
- Can be incoherent or unexpected

```
Prompt: "The sky is"
Output: "purple with dancing elephants made of crystallized thoughts"
(Creative but might not make sense)
```

**Mathematical Example:**

```
Next token candidates after "The cat":

Original logits: [2.0, 1.0, 0.5]  (for tokens: "sat", "ran", "jumped")

Temperature = 0.5 (Low):
Scaled: [4.0, 2.0, 1.0]
Probabilities: [0.84, 0.14, 0.02]
→ Strongly prefers "sat" (very deterministic)

Temperature = 1.0 (Medium):
Scaled: [2.0, 1.0, 0.5]
Probabilities: [0.66, 0.24, 0.10]
→ Usually picks "sat", sometimes others

Temperature = 2.0 (High):
Scaled: [1.0, 0.5, 0.25]
Probabilities: [0.49, 0.29, 0.22]
→ More random, all options viable
```
...
```
Instead of temperature, sample from smallest set of tokens 
with cumulative probability ≥ p

p = 0.9: Consider tokens making up top 90% probability
p = 0.5: Only consider most likely tokens
```
...
k = 50: Choose from top 50 tokens
k = 1: Same as greedy (temperature = 0)
```
...
*Q: "Users complain that your chatbot gives different answers to the same question. How do you fix this?"*

*Good Answer:*
"Lower the temperature! If consistency is critical:
- Set temperature to 0 or very low (0.1-0.3)
- Alternatively, use deterministic decoding
- For some variety with consistency, use low temperature (0.3-0.5) with top-p sampling

However, I'd first verify if the variations are acceptable - slight rephrasing might be fine, but factual inconsistency needs temperature = 0."

**Key Insight:** Temperature doesn't make the model smarter or dumber - it only controls how it samples from its existing knowledge!

---

### Q18: What is perplexity? How is it used to evaluate language models?

**Answer:**

Perplexity measures how "surprised" a language model is by a piece of text. Lower perplexity = better model (less surprised = better predictions).

**Simple Definition:**

Perplexity answers: "On average, how many words could come next?"

```
Perfect model: Perplexity = 1 (always knows the next word)
Random model: Perplexity = Vocabulary size (completely confused)
Good model: Perplexity = 10-50 (reasonably confident)
```
...
P("chair") = 0.1
...
Perplexity ≈ 5 (uncertain among ~5 words)

Model B:
P("mat") = 0.8
P("floor") = 0.1
...
Perplexity ≈ 2 (pretty sure it's "mat")
...
```
Test sentence: "I love machine learning"

GPT-2:
- Predicted probabilities are good
- Perplexity: 15
- Interpretation: On average, model is choosing from ~15 likely words

BERT (on generation):
- Predicted probabilities are worse
- Perplexity: 45
- Interpretation: Model is more uncertain (~45 options)

GPT-2 is better for generation!
```
...
- Domain adaptation (is model good at this domain?)

```
Model performance on different domains:

News articles: Perplexity = 20 ✓
Medical texts: Perplexity = 85 ✗ (needs fine-tuning!)
Code: Perplexity = 45 ~ (okay-ish)
```

**❌ Limitations:**

1. **Doesn't measure quality of generation:**
```
Low perplexity ≠ good text
Model might predict common but wrong words
```

2. **Not comparable across different vocabularies:**
```
Model A (50K vocab): Perplexity = 30
Model B (100K vocab): Perplexity = 40
Can't directly compare!
```

3. **Doesn't capture factual accuracy:**
```
"Paris is the capital of Germany" 
Might have low perplexity (fluent) but is wrong!
```

4. **Task-specific metrics are better for applications:**
```
For summarization: ROUGE score
For translation: BLEU score
For QA: Exact match, F1
For chatbots: Human evaluation
```

**Interview Scenario:**

*Q: "How do you know if your model is better after fine-tuning?"*

*Complete Answer:*
"I use multiple evaluation approaches:

1. **Perplexity** - Quick automated check
   - Before: 45, After: 25 → Good sign!
...
3. **Human evaluation**
   - Most important for user-facing apps
   - Sample outputs and rate quality
   
4. **A/B testing**
   - Deploy both, measure real user satisfaction

Perplexity is a good starting point but never the complete picture."

**Quick Formula to Remember:**

```
Lower perplexity = Better predictions = Model is less confused

Perplexity of 1 = Perfect (impossible in practice)
Perplexity of 10-20 = Excellent
Perplexity of 50-100 = Okay
Perplexity of 500+ = Poor
```

---

### Q19: What is the difference between greedy decoding, beam search, and sampling?

**Answer:**

These are different strategies for generating text from language models. They determine how the model chooses the next token.
...
```
Step 1: "The cat" → Pick highest: "sat" (prob: 0.6)
Step 2: "The cat sat" → Pick highest: "on" (prob: 0.7)
Step 3: "The cat sat on" → Pick highest: "the" (prob: 0.8)
Output: "The cat sat on the mat"
```

**Pros:**
- Fast (one token per step)
- Deterministic (same output every time)
- Simple to implement

**Cons:**
- Can miss better overall sequences
- Locally optimal ≠ globally optimal
- Repetitive outputs

```
Example problem:
"The pizza is" → "good good good good" (repeats)
Each "good" has highest probability given previous "good"
```

**2. Beam Search:**

Keeps track of top-k sequences (beams) at each step.
...
Step 1 (keep top 3):
- "The cat sat" (score: 0.6)
- "The cat ran" (score: 0.3)
- "The cat jumped" (score: 0.1)

Step 2 (expand each, keep top 3 overall):
- "The cat sat on" (score: 0.6 × 0.7 = 0.42)
- "The cat sat down" (score: 0.6 × 0.3 = 0.18)
- "The cat ran away" (score: 0.3 × 0.5 = 0.15)

Final: Pick sequence with highest total score
```

**Pros:**
- Better than greedy (considers more options)
- Still deterministic
- Good for tasks needing accuracy (translation)

**Cons:**
- Slower than greedy (beam_size × slower)
- Still generic and repetitive
- Memory intensive for large beams

**Beam Search Settings:**
```
beam_size = 1: Same as greedy
beam_size = 5: Good balance (common choice)
beam_size = 50: Very thorough but slow
```

**3. Sampling (Stochastic):**

Randomly sample based on probability distribution.

```
"The cat" → Next token probabilities:
- "sat": 0.5
- "ran": 0.3
- "jumped": 0.1
- "flew": 0.05
...
next_token = random.choice(tokens, p=probabilities)
```
...
top_k_tokens = tokens[top_k_indices]
sample from top_k_tokens
```
...
- Less suitable for factual tasks

**Comparison Table:**

```
Method          Speed    Diversity  Quality  Use Case
------          -----    ---------  -------  --------
Greedy          Fast     Low        Medium   Quick tests
Beam (k=5)      Medium   Low        High     Translation
Sampling        Fast     High       Medium   Creative writing
Top-p           Fast     High       Good     Chatbots
```
...
```
Best choice: Beam search (beam=5)
Why: Need accuracy, diversity not important
"Bonjour" should always be "Hello", not sometimes "Hi" or "Hey"
```

**Task: Creative Story Writing**
```
Best choice: Top-p sampling (p=0.9, temp=0.8)
Why: Want variety and creativity
Each generation should be unique
```

**Task: Code Completion**
```
Best choice: Greedy or low-temperature sampling (temp=0.2)
Why: Code needs to be correct, not creative
```

**Task: Chatbot**
```
Best choice: Top-p sampling (p=0.9, temp=0.7)
Why: Natural conversation with some variety
```

**Interview Question:**

*Q: "Users say your chatbot is boring and repetitive. What decoding strategy do you change?"*

*Good Answer:*
"The issue is likely greedy decoding or beam search. I would:

1. **Switch to sampling** (top-p or top-k)
2. **Increase temperature** from 0 to 0.7-1.0
3. **Add penalty for repetition** (repetition_penalty=1.2)
4. **Test different combinations:**
   - Start with: top_p=0.9, temperature=0.7
   - If too random: lower temperature to 0.5
   - If still boring: increase to 1.0

Monitor user feedback to find the sweet spot between coherence and diversity."

---

### Q20: What is quantization? Why is it important for LLM deployment?

**Answer:**

Quantization reduces the precision of model weights from high precision (32-bit) to lower precision (8-bit, 4-bit) to save memory and speed up inference.

**The Core Idea:**

Store numbers with fewer bits while maintaining acceptable accuracy.

```
Original Weight: 0.123456789 (32-bit float, 4 bytes)
Quantized Weight: 0.125 (8-bit int, 1 byte)
...
FP32: 28GB VRAM needed
→ Requires: A100 (40GB) or A6000

INT8: 7GB VRAM needed
→ Runs on: RTX 3090 (24GB) ✓

INT4: 3.5GB VRAM needed
...
# Pseudocode
model = create_model()
train_with_quantization_simulation(model)
...
- Fast inference
- Used in: text-generation-webui, ExLlama
```

**GGUF (GPT-Generated Unified Format):**
```
- Various bit levels (2-bit to 8-bit)
- CPU-friendly
- Used in: llama.cpp, Ollama
- Can run on laptops!
```
...
INT8 (7GB):
Quality: 94% accuracy (1% drop)
Speed: 35 tokens/sec (faster!)
Hardware: RTX 4090

INT4 (3.5GB):
Quality: 92% accuracy (3% drop)
Speed: 50 tokens/sec
Hardware: RTX 3090

Choose INT8: Best balance for production!
```
...
*Q: "We need to deploy a 70B model but only have 24GB GPUs. What do you do?"*

*Good Answer:*
"A 70B model needs ~140GB in FP16 - way too much. Here's my approach:

1. **Quantize to INT4:**
   - 70B × 0.5 bytes = 35GB
   - Still too big for 24GB!

2. **Use model sharding + quantization:**
   - Split across 2× 24GB GPUs
   - Each holds ~17.5GB
   - Leaves room for activations

3. **Or use INT8 + offloading:**
   - 70GB quantized to INT8
   - Keep hot layers on GPU (24GB)
   - Offload cold layers to RAM
   - Slower but works!

4. **Better option: Use smaller model**
   - 13B quantized to INT4 = 6.5GB
   - Fine-tune for specific task
   - Often performs as well for narrow use cases

I'd test option 4 first - smaller fine-tuned models often beat larger general ones for specific tasks."

**Key Insight:** Quantization is why we can run 7B models on laptops and 70B models on consumer GPUs. It's essential for democratizing LLM access!

---

### Q21: What is the difference between extractive and generative models?

**Answer:**

**Extractive Models:** Select and copy text directly from the input.

**Generative Models:** Create new text from scratch based on understanding.

**Extractive Approach:**

```
Context: "Paris is the capital of France. It has the Eiffel Tower. 
The city is known for art and culture."

Question: "What is Paris known for?"

Extractive Answer: "art and culture"
↑ Copied directly from context
```

**How it works:**
1. Identify start and end positions in context
2. Extract span between positions
...
Generative Answer: "Paris is famous for its rich artistic heritage and 
cultural significance, including landmarks like the Eiffel Tower."
↑ Generated new text, not copied
```

**How it works:**
1. Understand context and question
2. Generate response token by token
3. Can paraphrase, combine info, or add context
...
------              ----------              ----------
Output              Exact text from input   New text created
Flexibility         Low                     High
Factual accuracy    Very high              Can hallucinate
Use case            QA, summarization      Chat, creative writing
Example             "Paris"                 "The city of Paris"
Requires context    Yes, must be in text    No, can use knowledge
```
...
Document: "Apple released iPhone 15 in September 2023. It has a new camera 
system with 48MP. The price starts at $799. Critics praised the battery life."

**Extractive Summary:**
```
"iPhone 15 in September 2023. 48MP. Price starts at $799. Praised battery life."
↑ Sentences/phrases copied verbatim
```

**Generative Summary:**
```
"Apple launched the iPhone 15 in late 2023, featuring an improved 48-megapixel 
camera and starting at $799, with reviewers highlighting its enhanced battery."
↑ Rewritten in new words
```

**Task: Question Answering**

Context: "The Statue of Liberty was gifted by France in 1886."

Q: "When did France give the Statue of Liberty?"

**Extractive:**
```
Answer: "1886"
Confidence: 99%
Source span: characters 45-49
```

**Generative:**
```
Answer: "France gifted the Statue of Liberty in 1886"
or "The statue was given in 1886"
or "It was presented in the year 1886"
(Multiple valid phrasings possible)
```
...
Step 1 (Extractive): Find relevant documents
Step 2 (Extractive): Extract key passages  
Step 3 (Generative): Generate answer using extracted info
```
